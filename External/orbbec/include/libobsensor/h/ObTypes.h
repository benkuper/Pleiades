 ...
 */
typedef enum OBGlobalUnifiedPropertyType {
...
} OBGlobalUnifiedPropertyType,
    ob_global_unified_property_type;

/**
 * @brief 用于描述每一个属性的特性
 */
typedef struct OBGlobalUnifiedPropertyItem {
    OBGlobalUnifiedProperty     id;    // 属性id
    const char*                 name;  // 属性名字
    OBGlobalUnifiedPropertyType type;  // 属性类型
} OBGlobalUnifiedPropertyItem, ob_global_unified_property_item;
...
typedef enum {
    ...
    OBGyroSampleRate, ob_gyro_sample_rate, OBAccelSampleRate, ob_accel_sample_rate;
...
typedef enum {
    ...
    OBGyroFullScaleRange, ob_gyro_full_scale_range;
...
typedef enum {
    ...
    OBAccelFullScaleRange, ob_accel_full_scale_range;
